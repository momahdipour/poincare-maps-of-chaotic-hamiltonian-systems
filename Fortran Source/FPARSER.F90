...
CONTAINS
  ...
  FUNCTION evalf (i, Val) RESULT (res)
    ...
    DO IP=1,Comp(i)%ByteCodeSize
       ...
       CASE   (cAbs); Comp(i)%Stack(SP)=DABS(Comp(i)%Stack(SP))
       CASE   (cExp); Comp(i)%Stack(SP)=DEXP(Comp(i)%Stack(SP))
       CASE (cLog10); IF (Comp(i)%Stack(SP)<=0._rn) THEN; EvalErrType=3; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=DLOG10(Comp(i)%Stack(SP))
       CASE   (cLog); IF (Comp(i)%Stack(SP)<=0._rn) THEN; EvalErrType=3; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=DLOG(Comp(i)%Stack(SP))
       CASE  (cSqrt); IF (Comp(i)%Stack(SP)<0._rn) THEN; EvalErrType=3; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=DSQRT(Comp(i)%Stack(SP))
       CASE  (cSinh); Comp(i)%Stack(SP)=DSINH(Comp(i)%Stack(SP))
       CASE  (cCosh); Comp(i)%Stack(SP)=DCOSH(Comp(i)%Stack(SP))
       CASE  (cTanh); Comp(i)%Stack(SP)=DTANH(Comp(i)%Stack(SP))
       CASE   (cSin); Comp(i)%Stack(SP)=DSIN(Comp(i)%Stack(SP))
       CASE   (cCos); Comp(i)%Stack(SP)=DCOS(Comp(i)%Stack(SP))
       CASE   (cTan); Comp(i)%Stack(SP)=DTAN(Comp(i)%Stack(SP))
       CASE  (cAsin); IF ((Comp(i)%Stack(SP)<-1._rn).OR.(Comp(i)%Stack(SP)>1._rn)) THEN
                      EvalErrType=4; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=DASIN(Comp(i)%Stack(SP))
       CASE  (cAcos); IF ((Comp(i)%Stack(SP)<-1._rn).OR.(Comp(i)%Stack(SP)>1._rn)) THEN
                      EvalErrType=4; res=zero; RETURN; ENDIF
                      Comp(i)%Stack(SP)=DACOS(Comp(i)%Stack(SP))
       CASE  (cAtan); Comp(i)%Stack(SP)=DATAN(Comp(i)%Stack(SP))
